...
- Enh: Custom side menu via `frontend\modules\admin\Module::items` (mdmunir).
...
- Chg: Can customize return of `frontend\modules\admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `frontend\modules\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `frontend\modules\admin\Module::allowActions`. Set access control directly with `frontend\modules\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `frontend\modules\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
